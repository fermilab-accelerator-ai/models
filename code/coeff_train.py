...
for name, group in df2.groupby(df2.index.hour):
...
def fix_df(df):
	...
	df=df.round(decimals=5)  # round to one decimal after precision of devices
	df['diff'] = (df['B_VIMIN'] - df['B:VIMIN'])
	ddf=pd.DataFrame({'time': df['time_B:VIMIN'],'diff' : df['diff'], 'err' : df['B:IMINER']})
	ddf=ddf.set_index(pd.DatetimeIndex(ddf['time'])) # set index to time (not in-place operation)
	return ddf
...
for name, group in df_tot.groupby(df_tot.index.day):
    train = group.sample(n = sample_size, random_state = 42)
    x = train['err']
    y = train['diff']
    A = np.vstack([x, np.ones(len(x))]).T
...
'''
# useful reader fn for hour/day grouping
# also different across days, same hour:
# ms = [0.5502914057757847, -2.0795542955927337, -0.3397878311385049, 0.10500684328279193, 0.2914717222725448, 0.21870285383121824]